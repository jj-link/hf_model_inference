...
foreach ($version in @("3.12", "3.11", "3.10")) {
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $testVersion -match "Python (\d+)\.(\d+)") {
            ...
            Write-Host "OK: Found Python $majorVersion.$minorVersion (optimal for GPU support)" -ForegroundColor Green
...
if (-not $foundCompatible) {
    try {
        ...
        Write-Host "ERROR: Python not found. Please install Python 3.8-3.12 from https://python.org" -ForegroundColor Red
...
if ($majorVersion -lt 3 -or ($majorVersion -eq 3 -and $minorVersion -lt 8)) {
    ...
    Write-Host "ERROR: PYTHON VERSION TOO OLD" -ForegroundColor Red
    Write-Host "========================================" -ForegroundColor Red
    Write-Host ""
...
if ($majorVersion -eq 3 -and $minorVersion -ge 13) {
        Write-Host ""
        Write-Host "========================================" -ForegroundColor Red
        Write-Host "WARNING: INCOMPATIBLE PYTHON VERSION" -ForegroundColor Red
        Write-Host "========================================" -ForegroundColor Red
        Write-Host ""
        Write-Host "Current: Python $majorVersion.$minorVersion" -ForegroundColor Yellow
        ...
        if ($choice -eq "1") {
            ...
            Write-Host ""
            Write-Host "WARNING: Continuing with CPU-only installation..." -ForegroundColor Yellow
...
if (Test-Path $venvPath) {
    Write-Host "WARNING: Virtual environment already exists at '$venvPath'" -ForegroundColor Yellow
...
if (-not $env:VIRTUAL_ENV) {
    Write-Host "ERROR: Virtual environment activation did not set VIRTUAL_ENV" -ForegroundColor Red
...
Write-Host "Note: Your prompt may not show '(venv)' unless you run: . .\\setup.ps1" -ForegroundColor DarkGray
Write-Host ""
...
if ($forceCPU) {
    Write-Host "WARNING: Python 3.13+ detected - forcing CPU-only installation" -ForegroundColor Yellow
    $useCuda = $false
} else {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "WARNING: No NVIDIA GPU detected, will install CPU-only PyTorch" -ForegroundColor Yellow
            $useCuda = $false
        }
    } catch {
        Write-Host "WARNING: No NVIDIA GPU detected, will install CPU-only PyTorch" -ForegroundColor Yellow
...
if (-not $useCuda) {
    Write-Host "WARNING: bitsandbytes requires CUDA and may fail on CPU-only systems" -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "WARNING: Verification failed, but installation may still work" -ForegroundColor Yellow
} else {
